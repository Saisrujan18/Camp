				...
				<div className="mx-3">
					...
					{/* Members */}
						<div className={showMembers ? "space-y-2 m-2" : "hidden"}>
							{ showMembers && collabData.members.map((member) => getMember(member))}
						</div>
						<div className={showComments ? "space-y-2 m-2" : "hidden"}>
							{showComments && <Comments currentUser={user.email} comments={comments} addComment={addComment} author={collabData.author}/>}
						</div>
